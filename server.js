...
function buildFFmpegCommand(videoData) {
    ...
    // *** FIX 1: Use the actual duration from the payload (23 seconds in this case) ***
    ...
    console.log(`Generating FINAL PRODUCTION COMMAND for Job ${scriptId} for duration ${videoDuration}s.`);
    
    const args = [
        // Input 0: Main Video Stream (Blue Screen Placeholder)
        '-f', 'lavfi',
        // --- Duration is now dynamic ---
...
app.post('/render', async (req, res) => {
    ...
    const payload = { 
        ...
        full_script: fullScriptText, // FIXED: Now uses the combined scene descriptions
        environment_tag: videoData.animation_style || "2D", // FIXED: Now uses animation_style
        content_type: videoData.content_type || "cartoon",
        // FIXED: Now uses the nested script_analysis structure
