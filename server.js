...
app.post('/render', async (req, res) => {
    // --- 0. PRE-RENDER CLEANUP (Fixes file conflicts) ---
    try {
        ...
        console.warn('Error during pre-render cleanup:', e.message);
    }
    // ------------------------------------------------------------------
    ...
    // *** FIX: Remove the redundant "ffmpeg " prefix. The command should already be prefixed. ***
    const executionCommand = command.replace('output.mp4', outputFilePath); 
    // -----------------------------------------------------------------------------------------

    // 3. Run the FFmpeg process (120 second timeout)
    exec(executionCommand, { 
        timeout: 120000,
        shell: '/bin/sh', // Crucial stability fix
        env: process.env
        ...
        // Hypothetical Success URL (App Builder handles actual upload)
