// server.js - Storyloom Single-Process Dual-Purpose Server
// This file handles both HTTP requests and the background FFmpeg processing loop.
...
function executeFFmpeg(args, scriptId) {
    return new Promise((resolve, reject) => {
        // Extract duration from arguments for logging
...
async function processJob(job) {
    if (isProcessingJob) return; // Prevent concurrent job execution in this single process
    ...
    try {
        ...
        // If this times out, Render will restart the service, but the job will remain PENDING
        // and the loop will pick it up again on restart.
        ...
        // Cleanup happens inside uploadVideoToStorage's finally block
    }
}
...
async function fetchAndProcessJobs() {
    ...
    if (jobs && jobs.length > 0) {
        // Process the job asynchronously but wait for it to finish before checking again
